...
load("lev2db.Rda")
load("Lev3Int_b_s_355.Rda")
load("Lev3Int_e_s_355.Rda")
load("Lev3Int_b_s_532.Rda")
load("Lev3Int_e_s_532.Rda")
load("Lev3Int_b_s_1064.Rda")
load("Lev3Int_ang_s.Rda")
load("System.Rda")
load("Lev3pbl_s.Rda")
load("Lev3PartDep_s_355.Rda")
load("Lev3PartDep_s_532.Rda")
load("Lev3PartDep_s_1064.Rda")
...
for (p in 1:33)
{
  for (y in 2000:2019)
  {
    ...
    sss = paste0("DecJanFeb_", as.character(y - 1), "/", as.character(y))
    sss = c(sss, paste0(
      c("MarAprMay_", "JunJulAug_", "SepOctNov_"),
      as.character(y)
    ))
    colnames(app) = c("Season_Year", "Null")
    for (s in sss)
    {
      v = data.frame(s, NA)
      colnames(v) = c("Season_Year", "Null")
      app = rbind(app, v)
    }
    app = app[-1, ]
    
    db_e355 = db_e_s_355[db_e_s_355$Station == loc2[p] &
                           substr(
                             db_e_s_355$Season_Year,
                             nchar(db_e_s_355$Season_Year) - 3,
                             nchar(db_e_s_355$Season_Year)
                           ) == as.character(y), ]
    db_e355 = merge(db_e355, app, by = "Season_Year", all = TRUE)
    db_e532 = db_e_s_532[db_e_s_532$Station == loc2[p] &
                           substr(
                             db_e_s_532$Season_Year,
                             nchar(db_e_s_532$Season_Year) - 3,
                             nchar(db_e_s_532$Season_Year)
                           ) == as.character(y), ]
    db_e532 = merge(db_e532, app, by = "Season_Year", all = TRUE)
    db_b355 = db_b_s_355[db_b_s_355$Station == loc2[p] &
                           substr(
                             db_b_s_355$Season_Year,
                             nchar(db_b_s_355$Season_Year) - 3,
                             nchar(db_b_s_355$Season_Year)
                           ) == as.character(y), ]
    db_b355 = merge(db_b355, app, by = "Season_Year", all = TRUE)
    db_b532 = db_b_s_532[db_b_s_532$Station == loc2[p] &
                           substr(
                             db_b_s_532$Season_Year,
                             nchar(db_b_s_532$Season_Year) - 3,
                             nchar(db_b_s_532$Season_Year)
                           ) == as.character(y), ]
    db_b532 = merge(db_b532, app, by = "Season_Year", all = TRUE)
    db_b1064 = db_b_s_1064[db_b_s_1064$Station == loc2[p] &
                             substr(
                               db_b_s_1064$Season_Year,
                               nchar(db_b_s_1064$Season_Year) - 3,
                               nchar(db_b_s_1064$Season_Year)
                             ) == as.character(y), ]
    db_b1064 = merge(db_b1064, app, by = "Season_Year", all = TRUE)
    db_pbl = db_pbl_s_tot[db_pbl_s_tot$Station == loc2[p] &
                            substr(
                              db_pbl_s_tot$Season_Year,
                              nchar(db_pbl_s_tot$Season_Year) - 3,
                              nchar(db_pbl_s_tot$Season_Year)
                            ) == as.character(y), ]
    db_pbl = merge(db_pbl, app, by = "Season_Year", all = TRUE)
    db_pd355 = db_pd_s_355[db_pd_s_355$Station == loc2[p] &
                             substr(
                               db_pd_s_355$Season_Year,
                               nchar(db_pd_s_355$Season_Year) - 3,
                               nchar(db_pd_s_355$Season_Year)
                             ) == as.character(y), ]
    db_pd355 = merge(db_pd355, app, by = "Season_Year", all = TRUE)
    db_pd532 = db_pd_s_532[db_pd_s_532$Station == loc2[p] &
                             substr(
                               db_pd_s_532$Season_Year,
                               nchar(db_pd_s_532$Season_Year) - 3,
                               nchar(db_pd_s_532$Season_Year)
                             ) == as.character(y), ]
    db_pd532 = merge(db_pd532, app, by = "Season_Year", all = TRUE)
    db_pd1064 = db_pd_s_1064[db_pd_s_1064$Station == loc2[p] &
                               substr(
                                 db_pd_s_1064$Season_Year,
                                 nchar(db_pd_s_1064$Season_Year) - 3,
                                 nchar(db_pd_s_1064$Season_Year)
                               ) == as.character(y), ]
    db_pd1064 = merge(db_pd1064, app, by = "Season_Year", all = TRUE)
    db_ang = db_ang_s_tot[db_ang_s_tot$Station == loc2[p] &
                            substr(
                              db_ang_s_tot$Season_Year,
                              nchar(db_ang_s_tot$Season_Year) - 3,
                              nchar(db_ang_s_tot$Season_Year)
                            ) == as.character(y), ]
    db_ang = merge(db_ang, app, by = "Season_Year", all = TRUE)
    ...
    ncatt_put(
      ...
      attname = "title",
      attval = paste0(
        "Seasonal average integrated measurements - year ",
        as.character(y)
      )
    )
