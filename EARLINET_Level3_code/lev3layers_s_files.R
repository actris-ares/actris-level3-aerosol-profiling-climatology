  ...
  for (y in 2000:2019)
  {
    for (k in 1:4)
    {
      ...
      pd_vtt_355 = as.numeric(db_355$ParticleDep[db_355$Type == "b" |
                                                   db_355$PdF == 1])
      pd_vtt_532 = as.numeric(db_532$ParicletDep[db_532$Type == "b" |
                                                   db_532$PdF == 1])
      ...
      backscatter_counts_532 = hist(db_532$Backscatter, breaks = backscatter_breaks, plot =
                                      F)$counts
      backscatter_counts_1064 = hist(db_1064$Backscatter,
                                     breaks = backscatter_breaks,
                                     plot = F)$counts
    ...
    ncatt_put(
      ...
      attname = "title",
      attval = paste0(
        "Seasonal distribution of layer optical values - year ",
        as.character(y)
      )
    )
