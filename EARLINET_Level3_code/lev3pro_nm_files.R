...
load("lev2db.Rda")
load("ProfilesNormalMonth355.Rda")
load("ProfilesNormalMonth532.Rda")
load("ProfilesNormalMonth1064.Rda")
load("System.Rda")
...
for (i in 1:33)
{
  db_355 = db_prof_nm_355[db_prof_nm_355$Station == loc2[i], c(2:27)]
  colnames(db_355)[3:26] = paste0(colnames(db_355)[3:26], "_355")
  
  db_532 = db_prof_nm_532[db_prof_nm_532$Station == loc2[i], c(2:27)]
  colnames(db_532)[3:26] = paste0(colnames(db_532)[3:26], "_532")
  
  db_1064 = db_prof_nm_1064[db_prof_nm_1064$Place == loc2[i], c(2:19)]
  ...
  for (l in 1:12)
  {
    db_355_new = db_355[as.numeric(db_355$Month) == l, ]
    db_532_new = db_532[as.numeric(db_532$Month) == l, ]
    db_1064_new = db_1064[as.numeric(db_1064$Month) == l, ]
  ...
  source_name = lev2db[lev2db$Station == loc2[i], ]
  source_data = " "
  k = nrow(source_name)
  for (j in 1:k)
  {
    source_data = paste0(source_data, " ", source_name[j, 1])
  }
  ...
  nm_file = nc_create(
    paste0(
      "ACTRIS_AerRemSen_",
      loc2[i],
      "_Lev03_NorMon_0019_Pro_v02_qc030.nc"
    ),
    variabili,
    force_v4 = TRUE
  )
  ...
  ncvar_put(nm_file, station_altitude, station_alt[i])
  ...
  ncatt_put(nm_file, 0, attname = "title", attval = "Monthly average profile measurements - Normal")
  nc_close(nm_file)
  gw = getwd()
  file.rename(
    from = paste0(
